...
import { BookOpen, LayoutDashboard, NotebookPen, TableOfContents, Link2, Youtube, Twitter } from "lucide-react";
...
export function Sidebar({ ContentType, isOpen, setIsOpen }: SidebarProps) {
    ...
    return (
        <div
            className={`z-10 fixed top-0 left-0 h-full overflow-hidden bg-gray-600 text-white border-r border-stone-600 transition-all ease-in-out duration-700`}
            style={{
                width: isOpen ? '14rem' : '3.9rem',
            }}>
            <div className="flex justify-between pr-2 mb-10">   
                <Link href={"/"}>
            ...
            <div className="mt-20 space-y-4">
                <Link href="/dashboard/home" className="animate-slidein300">
                    <SidebarItem
                        text={isOpen ? ContentType.Dashboard : ""}
                        icon={<div className="w-5 ml-1 mr-1"><LayoutDashboard /></div>}
                    />
                </Link>
                {isOpen && <div
                ...
                <Link href="/dashboard/editor" className="animate-slidein500">
                    <SidebarItem
                        text={isOpen ? ContentType.NoteBook : ""}
                        icon={<NotebookPen />}
                    />
                </Link>
                <Link href="/dashboard/notes" className="animate-slidein900">
                    <SidebarItem
                        text={isOpen ? ContentType.SortNote : ""}
                        icon={<BookOpen />}
                    />
                </Link>
                <Link href="/dashboard/general" className="animate-slidein900">
                    <SidebarItem
                        text={isOpen ? ContentType.General : ""}
                        icon={<Link2 />}
                    />
                </Link>
                <Link href="/dashboard/youtube" className="animate-slidein700">
                    <SidebarItem
                        text={isOpen ? ContentType.Youtube : ""}
                        icon={<Youtube />}
                    />
                </Link>
                <Link href="/dashboard/twitter" className="animate-slidein900">
                    <SidebarItem
